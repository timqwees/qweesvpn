  ...
  <div class="min-h-screen flex flex-col w-full container mx-auto">
    ...
    <main class="flex sm:my-2 w-full">
      <div class=" w-full text-white">
        <section
          class="overflow-hidden relative flex flex-col gap-6 justify-center pt-[95px] pb-4 box-border w-full min-h-[100dvh] px-4 bg-gradient-to-t from-black via-green-950 to-black">
          <!-- success icon -->
          <div class="w-full flex justify-center items-center">
            <div
              class="<?php echo $paymentStatus['paid'] ? 'bg-green-500/20' : 'bg-yellow-500/20'; ?> relative flex items-center justify-center p-6 aspect-square rounded-full">
              <i
                class="fas <?php echo $paymentStatus['paid'] ? 'fa-check text-green-400' : 'fa-clock text-yellow-400'; ?> text-4xl"></i>
            </div>
          </div>
          ...
          <div class="bg-white/10 rounded-2xl p-4 mb-4">
            <div class="flex justify-between mb-2">
              ...
              <span id="payment-id text-end"><?php echo htmlspecialchars($paymentId ?? 'TEST_PAYMENT'); ?></span>
            </div>
            <div class="flex justify-between mb-2">
              <span class="text-white/70">Статус:</span>
              <span
                class="<?php echo $paymentStatus['paid'] ? 'text-green-400' : ($paymentStatus['status'] === 'canceled' ? 'text-red-400' : 'text-yellow-400'); ?>">
                <?php
                if ($paymentStatus['paid']) {
                  echo 'Оплачено';
                } elseif ($paymentStatus['status'] === 'pending') {
