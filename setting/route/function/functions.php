...
class Functions
{
  ...
  public function on_Main(
    $path = '/public/pages/telegram/main/index.php'
  ) {
  ...
  /**
   * Обработчик события входа пользователя через Telegram.
   *
   * Создаёт или обновляет пользователя в БД по данным, полученным из запроса Telegram.
   * Если пользователь новый — сохраняет его, пишет лог и устанавливает сессию.
   * В любом случае возвращает JSON-ответ с результатом: создан ли клиент впервые.
   *
   * @return void
   */
  ...
  public static function Listener()
  {
    ...
    $ui = [
      'tg_id' => isset($user['id']) ? $user['id'] : 0,
      'tg_username' => isset($user['username']) ? strval($user['username']) : null,
      'tg_first_name' => isset($user['first_name']) ? strval($user['first_name']) : null,
      'tg_last_name' => isset($user['last_name']) ? strval($user['last_name']) : null,
    ];

    $result = Database::send(
      'SELECT tg_id FROM vpn_users WHERE tg_id = ?',
      [strval($ui['tg_id'])]
    );
    $userExists = false;
    // Улучшенная проверка: проверяем, что результат - это массив и не пустой, и содержит нужное поле
    if (is_array($result) && !empty($result) && isset($result[0]['tg_id'])) {
      $userExists = strval($result[0]['tg_id']) === strval($ui['tg_id']);
    }

    // Проверяем, есть ли my_refer_link для пользователя, если нет — генерируем новый.
    $my_refer_link = (new self)->GenerationRefer();

    // Получаем CID данные клиента
    $cidData = self::getClientCID();

    if (!$userExists) {
      $insertResult = Database::send(
        "INSERT INTO vpn_users (tg_id, tg_username, tg_first_name, tg_last_name, vpn_uuid, vpn_subscription, vpn_status, vpn_freekey, kassa_id, vpn_date_count, vpn_divece_count, card_token, yk_autopay_active, yk_autopay_id, yk_autopay_date, vpn_amount, refer_link, my_refer_count, my_refer_link, cid_ip, cid_user_agent, cid_device_type, cid_os) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [
          strval($ui['tg_id']),
          strval($ui['tg_username'] ?? ''),
          strval($ui['tg_first_name'] ?? ''),
          strval($ui['tg_last_name'] ?? ''),
          '',/*uuid*/
          '',/*sub*/
          '',/*status*/
          'no_used',/* Новый клиент, еще ничего не активировал */
          '',/*kassa_id*/
          '',/*vpn_date_count*/
          '', /*vpn_divece_count*/
          '',/*  card_token - NOT NULL в схеме, поэтому пустая строка вместо null*/
          0,/* yk_autopay_active (off)*/
          '',/* yk_autopay_id - DEFAULT '' в схеме, поэтому пустая строка*/
          '',/* yk_autopay_date - DEFAULT '' в схеме, поэтому пустая строка*/
          '',/* vpn_amount - NOT NULL DEFAULT '' в схеме, поэтому пустая строка вместо null*/
          '',/* refer_link (по умолчанию пусто, не определяем при регистрации) */
          0,/* my_refer_count (0)*/
          strval($my_refer_link),/* my_refer_link (уникальный реферальный код для этого пользователя) */
          strval($cidData['ip'] ?? ''),/* cid_ip */
          strval($cidData['user_agent'] ?? ''),/* cid_user_agent */
          strval($cidData['device_type'] ?? ''),/* cid_device_type */
          strval($cidData['os'] ?? '')/* cid_os */
        ]
      );
      ...
      if ($insertResult === false) {
        ...
        $errorDetails = [
          'tg_id' => $ui['tg_id'],
          'tg_username' => $ui['tg_username'],
          'tg_first_name' => $ui['tg_first_name'],
          'tg_last_name' => $ui['tg_last_name'],
          'my_refer_link' => $my_refer_link
        ];
        file_put_contents(
          $_ENV['LOG_FILE_NAME'] ?? 'coravpn.log',
          sprintf(
            "[%s] [ОШИБКА - СОЗДАНИЕ ПОЛЬЗОВАТЕЛЯ] Listener: Не удалось создать пользователя: ID=%s, Username=@%s, Имя=%s. Детали: %s\n",
            date('Y-m-d H:i:s'),
            $ui['tg_id'],
            $ui['tg_username'] ?? '',
            $ui['tg_first_name'] ?? '',
        ...
        // Также проверяем логи ошибок PHP
        ...
        echo json_encode(['error' => 'Ошибка создания пользователя', 'details' => 'Проверьте логи для деталей']);
        exit;
      }
      // ЛОГИРУЕМ
      file_put_contents(
        $_ENV['LOG_FILE_NAME'] ?? 'coravpn.log',
        sprintf(
          "[%s] [НОВЫЙ ПОЛЬЗОВАТЕЛЬ] Добавлен новый пользователь: ID=%s, Username=@%s, Имя=%s\n",
          date('Y-m-d H:i:s'),
          $ui['tg_id'],
          $ui['tg_username'],
          $ui['tg_first_name']
      ...
      // Ищем pending реферальный код по IP/CID данным (из БД)
      $pendingReferData = self::findPendingReferCodeByCID($cidData);
      $pendingReferCode = null;
      $pendingReferId = null;
      $codeAlreadyActivated = false;

      if ($pendingReferData && !empty($pendingReferData['refer_code'])) {
        // Проверяем, не активирован ли код уже другим устройством
        if (isset($pendingReferData['activated']) && $pendingReferData['activated'] == 1) {
          $codeAlreadyActivated = true;
          file_put_contents(
            $_ENV['LOG_FILE_NAME'] ?? 'coravpn.log',
            sprintf(
              "[%s] [РЕФЕРАЛ УЖЕ АКТИВИРОВАН] Код pending_refer_codes id=%s уже активирован другим устройством (tg_id=%s). Текущий пользователь tg_id=%s, IP=%s\n",
              date('Y-m-d H:i:s'),
              $pendingReferData['id'],
              $pendingReferData['activated_tg_id'] ?? 'unknown',
              $ui['tg_id'],
              $cidData['ip'] ?? 'unknown'
            ),
            FILE_APPEND
          ...
          $pendingReferCode = $pendingReferData['refer_code'];
          $pendingReferId = $pendingReferData['id'];
        }
      } else {
        // Также проверяем cookie/сессию для обратной совместимости
        if (isset($_COOKIE['pending_refer_code']) && !empty($_COOKIE['pending_refer_code'])) {
          $pendingReferCode = $_COOKIE['pending_refer_code'];
        } elseif (isset($_SESSION['pending_refer_code']) && !empty($_SESSION['pending_refer_code'])) {
          $pendingReferCode = $_SESSION['pending_refer_code'];
        }
      }

      // Если есть сохраненный реферальный код и он не активирован - активируем его
      if (!empty($pendingReferCode) && !$codeAlreadyActivated) {
        $functions = new self();
        $referActivated = $functions->setRefer($ui['tg_id'], $pendingReferCode, '/profile', true); // true = не делать редирект

        if ($referActivated) {
          // Атомарно помечаем pending код как активированный в БД
          // Если код уже активирован другим устройством - markPendingReferCodeAsActivated вернет false
          if ($pendingReferId) {
            $marked = self::markPendingReferCodeAsActivated($pendingReferId, $ui['tg_id']);

            if (!$marked) {
              // Код уже активирован другим устройством - коллизия!
              file_put_contents(
                $_ENV['LOG_FILE_NAME'] ?? 'coravpn.log',
                sprintf(
                  "[%s] [КОЛЛИЗИЯ РЕФЕРАЛ] Не удалось пометить код как активированный - возможно, он уже активирован другим устройством. pending_id=%s, tg_id=%s, IP=%s\n",
                  date('Y-m-d H:i:s'),
                  $pendingReferId,
                  $ui['tg_id'],
                  $cidData['ip'] ?? 'unknown'
          ...
          // Удаляем сохраненный код из cookie/сессии (для обратной совместимости)
          $domain = $_SERVER['HTTP_HOST'] ?? '';
          $secure = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') || ($_SERVER['SERVER_PORT'] ?? null) == 443;
          setcookie('pending_refer_code', '', time() - 3600, '/', $domain, $secure, true);
          unset($_SESSION['pending_refer_code']);
          unset($_SESSION['pending_cid_data']);

          file_put_contents(
            $_ENV['LOG_FILE_NAME'] ?? 'coravpn.log',
            sprintf(
              "[%s] [РЕФЕРАЛ АКТИВИРОВАН ПРИ РЕГИСТРАЦИИ] Новый пользователь tg_id=%s активировал сохраненный реферальный код %s (найден по IP=%s, device=%s, os=%s)\n",
              date('Y-m-d H:i:s'),
              $ui['tg_id'],
              $pendingReferCode,
              $cidData['ip'] ?? 'unknown',
              $cidData['device_type'] ?? 'unknown',
              $cidData['os'] ?? 'unknown'
            ),
            FILE_APPEND
          );
      ...
      // Сохраняем текущие CID данные
      self::saveClientCID($ui['tg_id'], $cidData);

      $_SESSION['client'] = intval($ui['tg_id']);
      echo json_encode(['status' => 'ok', 'created' => true]);
      return;
    } else {
      // Обновляем CID данные для существующего пользователя
      self::saveClientCID($ui['tg_id'], $cidData);
      ...
      // Если есть сохраненный реферальный код - проверяем, можно ли его активировать
      if (!empty($pendingReferCode) && !$codeAlreadyActivated) {
        $client = (new self)->client($ui['tg_id']);
        // Активируем только если у пользователя еще нет активной реферальной ссылки
        if (empty($client['refer_link'])) {
          ...
          if ($referActivated) {
            ...
            file_put_contents(
              $_ENV['LOG_FILE_NAME'] ?? 'coravpn.log',
              sprintf(
                "[%s] [РЕФЕРАЛ АКТИВИРОВАН ПРИ ВХОДЕ] Существующий пользователь tg_id=%s активировал сохраненный реферальный код %s (найден по IP=%s, device=%s, os=%s)\n",
                date('Y-m-d H:i:s'),
                $ui['tg_id'],
                $pendingReferCode,
                $cidData['ip'] ?? 'unknown',
          ...
          // У пользователя уже есть реферальная ссылка - помечаем код как использованный (если он был найден)
          if ($pendingReferId) {
            self::markPendingReferCodeAsActivated($pendingReferId, $ui['tg_id']); // Помечаем как использованный
          }
      ...
      $_SESSION['client'] = intval($ui['tg_id']);
      echo json_encode(['status' => 'ok', 'created' => false]);
      return;
    }
  }
  ...
  /**
   * Получить расширенную информацию о клиенте из БД, собрать состояние по подписке и бесплатному ключу.
   *
   * Производит запрос к таблице пользователей, рассчитывает состояние подписки (активна, истекла, оставшееся время),
   * формирует параметры подключения (например, ссылку vless), рассчитывает и возвращает статус для кнопок (buy/free).
   *
   * @param string|int $tg_id - Telegram ID пользователя
   * @return array             - Полная информация о клиенте для генерации интерфейса
   */
  public function client($tg_id): array
  {
    $row = Database::send('SELECT * FROM vpn_users WHERE tg_id = ?', [$tg_id]);
    if (!is_array($row) || empty($row)) {
      ...
      return [
        ...
        'vpn_vless' => '',
        'vpn_status' => 'offline',
        'vpn_expiryTime' => 'Нет подписки',
        'vpn_expiryDate' => 'Ожидание...',
        'vpn_freekey' => 'no_used',
        'kassa_id' => '',
        'refer_link' => '',
        'my_refer_count' => 0,
        ...
        'vpn_date_count' => 0,
        'vpn_divece_count' => 0,
        'vpn_amount' => null, // возвращаем поле vpn_amount
        'card_token' => null, // Добавим card_token в ответ по-умолчанию
        'yk_autopay_active' => 0,
        'yk_autopay_id' => null,
        'yk_autopay_date' => null,
        'refer_discount' => 0
      ];
    }

    $user = $row[0];
    $expiry_ms = intval($user['vpn_status'] ?? 0);
    $now = time();
    $expiryTimeStr = '';
    $expiryDateStr = '';
    $status = 'offline';
    ...
    if ($expiry_ms > 0) {
      ...
      if ($time_left > 0) {
        ...
        if ($time_left < 86400) {
          ...
          $expiryDateStr = $isToday ? "Сегодня до $humanTime" : "Завтра до $humanTime";
        } else {
          $expiryDateStr = 'до ' . date('d.m.Y', $expiry_s);
        }
        $days = floor($time_left / 86400);
        $hours = floor(($time_left % 86400) / 3600);
        $minutes = floor(($time_left % 3600) / 60);
        // Формируем правильную форму слова для "дней"
        if ($days == 1) {
          $daysStr = "$days день";
        ...
        $expiryTimeStr = trim(sprintf('%s %02d ч %02d м', $daysStr, $hours, $minutes));
      } else {
        $status = 'offline';
        $expiryDateStr = 'Истекла';
        $expiryTimeStr = 'Подписка отсувствует!';
        Database::send(
      ...
      $expiryDateStr = 'Нет подписки';
    }

    $vless_uuid = strval($user['vpn_uuid'] ?? '');

    $vless_domain = isset($_ENV['VLESS_SERVER']) ? $_ENV['VLESS_SERVER'] : 'nl.coravpn.online';
    $vless_port = isset($_ENV['VLESS_PORT']) ? $_ENV['VLESS_PORT'] : '443';
    $vless_path = isset($_ENV['VLESS_PATH']) ? $_ENV['VLESS_PATH'] : '/vless';
    $vless_sni = isset($_ENV['VLESS_SNI']) ? $_ENV['VLESS_SNI'] : $vless_domain;
    $vless_user = ($user['tg_username'] ?? 'Client');
    $vless_label = rawurlencode("{$_SERVER['VLESS_PREFIX']}-{$vless_user}");

    $vless_url = '';
    if ($vless_uuid) {
      $vless_url = "vless://{$vless_uuid}@{$vless_domain}:{$vless_port}?" .
        "alpn=h2%2Chttp%2F1.1" .
        "&encryption=none" .
        "&fp=chrome" .
        "&host={$vless_domain}" .
        "&path=" . rawurlencode($vless_path) .
        "&security=tls" .
        "&sni={$vless_sni}" .
        "&type=ws" .
        "#{$vless_label}";
    ...
    // Как работает:
    // Определяет состояние бесплатного ключа (vpn_freekey) пользователя для VPN.
    // 1. Получает текущее значение vpn_freekey из данных пользователя, иначе считает "used" (уже использован/неактуален).
    $vpn_freekey = isset($user['vpn_freekey']) ? strval($user['vpn_freekey']) : 'used';

    // 2. Список допустимых состояний:
    $free_states = [
      'no_used',   // Новый клиент, можно "Подключить бесплатно!" (trial)
      'used',      // Бесплатник пробовали, доступна только покупка
      'buy',       // Куплено, не подключено
      'used_free', // Бесплатная активирована, но не подключено в v2Ray
    ];

    // 3. Если значение не входит в список допустимых, ставим "used" (как "нет бесплатных опций, только покупка").
    if (!in_array($vpn_freekey, $free_states, true)) {
      $vpn_freekey = 'used';
    }
    ...
    return [
      ...
      'my_refer_count' => isset($user['my_refer_count']) ? intval($user['my_refer_count']) : 0, // количество приглашённых
      'my_refer_link' => isset($user['my_refer_link']) ? strval($user['my_refer_link']) : '', // собственная реферальная ссылка
      'refer_discount' => isset($user['refer_discount']) ? intval($user['refer_discount']) : 0, // скидка от реферальной ссылки
      'vpn_vless' => $vless_url,
      'vpn_status' => $status,
      'vpn_expiryTime' => $expiryTimeStr,
      'vpn_expiryDate' => $expiryDateStr,
      'yk_autopay_active' => $user['yk_autopay_active'] ?? 0,
      'yk_autopay_id' => $user['yk_autopay_id'] ?? null,
      'yk_autopay_date' => $user['yk_autopay_date'] ?? null
    ];
  }
