...
//==================================================================================================//REFER
Routes::get('/ref={refer}', function ($refer) {
  // Получаем CID данные клиента, который открыл реферальную ссылку
  ...
  // Ищем клиента по CID данным (многоуровневый поиск)
  $foundClient = Functions::findClientByCID($cidData);

  // Извлекаем код рефера из параметра ref=xxxxxxx
  $referCode = strpos($refer, 'ref=') === 0 ? substr($refer, strlen('ref=')) : $refer;

  if ($foundClient && !empty($foundClient['tg_id'])) {
    // Клиент найден по CID - проверяем, существует ли он в БД
    $client_id = $foundClient['tg_id'];

    // ВАЖНО: Проверяем существование пользователя в БД перед активацией
    $clientCheck = Database::send(
      'SELECT tg_id FROM vpn_users WHERE tg_id = ? LIMIT 1',
      [strval($client_id)]
    );

    $userExists = false;
    if (is_array($clientCheck) && !empty($clientCheck) && isset($clientCheck[0]['tg_id'])) {
      $userExists = strval($clientCheck[0]['tg_id']) === strval($client_id);
    }

    if ($userExists) {
      // Пользователь существует в БД - выдаем реферальную подписку
      $functions = new Functions();
      $referActivated = $functions->setRefer($client_id, $referCode, '/ref_success');

      if ($referActivated) {
        // Обновляем CID данные найденного клиента (на случай, если они изменились)
        Functions::saveClientCID($client_id, $cidData);

        // ЛОГИРУЕМ
        file_put_contents(
          $_ENV['LOG_FILE_NAME'] ?? 'coravpn.log',
          sprintf(
            "[%s] [REFER BY CID - SUCCESS] Клиент найден по CID и получил реферальную подписку. tg_id=%s, ip=%s, device=%s, os=%s, refer_code=%s\n",
            date('Y-m-d H:i:s'),
            $client_id,
            $cidData['ip'] ?? 'unknown',
        ...
        // Активация не удалась - сохраняем код в БД для последующей активации по IP
        Functions::savePendingReferCode($referCode, $cidData, 30);

        file_put_contents(
          $_ENV['LOG_FILE_NAME'] ?? 'coravpn.log',
          sprintf(
            "[%s] [REFER BY CID - PENDING] Клиент найден по CID, но активация не удалась. Код сохранен в БД для активации при следующем входе. tg_id=%s, ip=%s, refer_code=%s\n",
            date('Y-m-d H:i:s'),
            $client_id,
            $cidData['ip'] ?? 'unknown',
            $referCode
      ...
      // Пользователь найден по CID, но не существует в БД (удален) - сохраняем код в БД
      Functions::savePendingReferCode($referCode, $cidData, 30);

      file_put_contents(
        $_ENV['LOG_FILE_NAME'] ?? 'coravpn.log',
        sprintf(
          "[%s] [REFER BY CID - PENDING] Клиент найден по CID, но не существует в БД (возможно удален). Код сохранен в БД для активации при регистрации. tg_id=%s, ip=%s, refer_code=%s\n",
          date('Y-m-d H:i:s'),
          $client_id,
          $cidData['ip'] ?? 'unknown',
          $referCode
    ...
    // Клиент не найден по CID - сохраняем реферальный код в БД для активации при первом входе через Telegram
    $referCode = strpos($refer, 'ref=') === 0 ? substr($refer, strlen('ref=')) : $refer;

    // Сохраняем реферальный код в БД с привязкой к IP/CID данным
    Functions::savePendingReferCode($referCode, $cidData, 30);

    // Логируем сохранение реферального кода в БД
    file_put_contents(
      $_ENV['LOG_FILE_NAME'] ?? 'coravpn.log',
      sprintf(
        "[%s] [REFER BY CID - PENDING] Клиент не найден по CID. Реферальный код сохранен в БД для активации при первом входе. ip=%s, device=%s, os=%s, refer_code=%s\n",
        date('Y-m-d H:i:s'),
        $cidData['ip'] ?? 'unknown',
        $cidData['device_type'] ?? 'unknown',
        $cidData['os'] ?? 'unknown',
  ...
  Network::onRedirect('/ref_success');
});
//==================================================================================================//uprefers
